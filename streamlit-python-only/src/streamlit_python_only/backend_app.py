...
from streamlit_python_only.events import now_iso, sse
...
from streamlit_python_only.settings import Settings, ensure_runtime_dirs, get_settings
...
def create_backend_app(
    ...
    app = FastAPI(title="AI Technical Assistant Backend", version="0.2.0")
    app.add_middleware(
        CORSMiddleware,
    ...
    async def list_terminals(
        ...
        params = {"alive_only": str(alive_only).lower()}
        if run_id:
            params["run_id"] = run_id
        if session_id:
            params["session_id"] = session_id
        async with sidecar_client(timeout=30.0) as client:
            response = await client.get("/v1/terminals", params=params)
            if response.status_code >= 400:
                _raise_sidecar_error(response)
            return response.json()
    ...
    async def terminal_snapshot(terminal_id: str) -> dict[str, Any]:
        async with sidecar_client(timeout=30.0) as client:
            response = await client.get(f"/v1/terminals/{terminal_id}/snapshot")
            if response.status_code >= 400:
                _raise_sidecar_error(response)
            return response.json()

    @app.post("/v1/terminals/{terminal_id}/wait_for_output")
    async def wait_for_terminal_output(terminal_id: str, request: TerminalWaitRequest) -> dict[str, Any]:
        async with sidecar_client(timeout=max(30.0, request.timeout_ms / 1000.0 + 5.0)) as client:
            response = await client.post(
                f"/v1/terminals/{terminal_id}/wait_for_output",
    ...
    async def resolve_terminal(request: TerminalResolveRequest) -> dict[str, Any]:
        async with sidecar_client(timeout=30.0) as client:
            response = await client.post("/v1/terminals/resolve", json=request.model_dump())
            if response.status_code >= 400:
                _raise_sidecar_error(response)
            return response.json()
    ...
    async def close_run_terminals(run_id: str, reason: str = Query(default="run_cancelled")) -> dict[str, Any]:
        async with sidecar_client(timeout=30.0) as client:
            response = await client.post(f"/v1/runs/{run_id}/terminals/close", params={"reason": reason})
            if response.status_code >= 400:
                _raise_sidecar_error(response)
            result = response.json()
            persist_terminal_events(result.get("events", []))
