...
from streamlit_python_only.events import sse
from streamlit_python_only.providers import list_available_models, resolve_provider
...
from streamlit_python_only.schemas import (
...
from streamlit_python_only.terminal_manager import get_terminal_manager
from streamlit_python_only.providers import build_chat_model
...
def create_sidecar_app(
    ...
    app = FastAPI(title="AI Technical Assistant Sidecar", version="0.1.0")
    app.add_middleware(
        CORSMiddleware,
    ...
    effective_provider_resolver = runtime.deps.provider_resolver
    ...
    def capabilities() -> dict[str, Any]:
        provider = effective_provider_resolver(None, None)
        return {
    ...
    def models() -> dict[str, Any]:
        return list_available_models(provider=effective_provider_resolver(None, None))
    ...
    def list_terminals(
        ...
        return terminal_manager.list_terminals(run_id=run_id, session_id=session_id, alive_only=alive_only)
    ...
    @app.get("/v1/terminals/{terminal_id}/snapshot")
    def terminal_snapshot(terminal_id: str) -> dict[str, Any]:
        try:
            snapshot = terminal_manager.terminal_snapshot(terminal_id)
            return {"snapshot": snapshot, "terminal": terminal_manager.get_terminal(terminal_id)["terminal"]}
        except KeyError as exc:
            raise HTTPException(status_code=404, detail="Terminal not found") from exc

    @app.post("/v1/terminals/{terminal_id}/wait_for_output")
    def wait_for_terminal_output(terminal_id: str, request: TerminalWaitRequest) -> dict[str, Any]:
        try:
            return terminal_manager.wait_for_output(
                terminal_id,
    ...
    @app.post("/v1/terminals/resolve")
    def resolve_terminal(request: TerminalResolveRequest) -> dict[str, Any]:
        try:
            return terminal_manager.resolve_terminal_reference(
                request.terminal_id,
                run_id=request.run_id,
                session_id=request.session_id,
                require_alive=request.require_alive,
            )
        except KeyError as exc:
            raise HTTPException(status_code=404, detail="Terminal not found") from exc
        except ValueError as exc:
    ...
    @app.post("/v1/runs/{run_id}/terminals/close")
    def close_run_terminals(run_id: str, reason: str = Query(default="run_cancelled")) -> dict[str, Any]:
        return terminal_manager.close_run_terminals(run_id, reason=reason)
