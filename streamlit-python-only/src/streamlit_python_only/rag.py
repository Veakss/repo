...
RAG_CONTEXT_RECORD_TYPE = "rag_lookup_context"
RAG_STRONG_HIT_SCORE = 2.5
...
def compact_snippet(text: str, limit: int = 220) -> str:
    normalized = normalize_whitespace_block(text)
    if len(normalized) <= limit:
        return normalized
    return normalized[: limit - 1].rstrip() + "…"


def normalize_whitespace_block(value: str) -> str:
...
def score_chunk_details(
    ...
    preferred_terms = ordered_unique([term.lower() for term in preferred_terms or []])
    preferred_paths = ordered_unique([term.lower() for term in preferred_paths or []])
    ...
    if len(text.strip()) < 60:
        score -= 0.35
    preferred_term_matches = [term for term in preferred_terms if term in hay and term not in matched_terms]
    if preferred_term_matches:
        matched_terms.extend(preferred_term_matches[:3])
        score += min(1.2, len(preferred_term_matches) * 0.3)
    normalized_path = str(path or "").lower()
    if normalized_path and any(item in normalized_path for item in preferred_paths):
        score += 0.6
    score = max(0.0, round(score, 3))
    return {
        "score": score,
        "matchedTerms": ordered_unique(matched_terms),
        "strength": "strong" if score >= RAG_STRONG_HIT_SCORE else "weak",
    }


def is_followup_query(query: str, query_tokens: list[str]) -> bool:
    lowered = str(query or "").strip().lower()
    if not lowered:
        return False
    if lowered.startswith(RAG_FOLLOWUP_PREFIXES):
        return True
    if len(query_tokens) <= 5 and any(token in query_tokens for token in ("status", "checkpoint", "phase", "suite", "ensuite", "there", "that")):
        return True
    return False


def build_citation_label(citation: dict[str, Any]) -> str:
...
def render_lookup_for_model(response: dict[str, Any]) -> str:
    ...
    parts = [
        "RAG context:",
        f"Hit strength: strong={int(meta.get('strongHitCount', 0))} weak={int(meta.get('weakHitCount', 0))}",
    ]
    if meta.get("followupContextUsed"):
        parts.append("Follow-up context from the previous RAG turn was reused for ranking.")
    if meta.get("matchedTerms"):
        parts.append(f"Matched terms: {', '.join(meta.get('matchedTerms', [])[:8])}")
    if int(meta.get("strongHitCount", 0)) <= 0:
        parts.append("Retrieved context is weak. Answer cautiously and say when the documents are insufficient.")
    for hit in response.get("hits", [])[:5]:
        citation = hit.get("citation", {})
        label = build_citation_label(citation)
        matched = ", ".join(hit.get("matchedTerms", [])[:6]) or "-"
        parts.append(
            f"- [{hit.get('strength', 'weak')}] {label} score={float(hit.get('score', 0.0)):.2f} matched={matched}\n"
            f"{compact_snippet(hit.get('snippet', ''), limit=280)}"
        )
    return "\n\n".join(parts)
...
class RagService:
    ...
    def _latest_lookup_context(self, session_id: str) -> dict[str, Any] | None:
        return self._clean(
            self.db.rag_memory_entries.find_one(
                {"session_id": session_id, "record_type": RAG_CONTEXT_RECORD_TYPE},
                sort=[("updatedAt", -1)],
            )
        )

    def _store_lookup_context(self, session_id: str, response: dict[str, Any]) -> None:
        hits = response.get("hits", []) if isinstance(response.get("hits"), list) else []
        meta = response.get("meta", {}) if isinstance(response.get("meta"), dict) else {}
        if not hits or int(meta.get("strongHitCount", 0)) <= 0:
            return
        context = {
            "record_type": RAG_CONTEXT_RECORD_TYPE,
            "session_id": session_id,
            "query": response.get("query", ""),
            "queryTokens": meta.get("queryTokens", []),
            "matchedTerms": meta.get("matchedTerms", []),
            "topDocumentPaths": meta.get("topDocumentPaths", []),
            "hits": [
                {
                    "chunkId": hit.get("chunkId"),
                    "fileId": hit.get("fileId"),
                    "path": hit.get("citation", {}).get("path"),
                    "lineStart": hit.get("citation", {}).get("lineStart"),
                    "lineEnd": hit.get("citation", {}).get("lineEnd"),
                    "matchedTerms": hit.get("matchedTerms", []),
                    "score": hit.get("score"),
    ...
    def _context_for_query(self, session_id: str | None, query: str, query_tokens: list[str]) -> dict[str, Any] | None:
        if not session_id or not is_followup_query(query, query_tokens):
            return None
        context = self._latest_lookup_context(session_id)
        if not context:
            return None
        context_terms = [term for term in context.get("matchedTerms", []) if str(term).strip()]
        path_terms: list[str] = []
        for path in context.get("topDocumentPaths", []):
            path_terms.extend(tokenize(Path(str(path)).stem))
        merged_terms = ordered_unique(context_terms + path_terms)
        if not merged_terms:
            return None
        return {
            "matchedTerms": merged_terms[:8],
            "topDocumentPaths": ordered_unique([str(path) for path in context.get("topDocumentPaths", [])])[:4],
    ...
    def lookup(self, question: str, session_id: str | None = None, scope: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        followup_context = self._context_for_query(session_id, query, query_tokens)
        preferred_terms = list(followup_context.get("matchedTerms", [])) if followup_context else []
        preferred_paths = list(followup_context.get("topDocumentPaths", [])) if followup_context else []
        effective_query_tokens = ordered_unique(query_tokens + preferred_terms)
        ...
        if "session_memory" in source_priority and scope.get("session_memory", True) and session_id:
            ...
            if not memory_state["config"]["enabled"]:
                ...
                for path, text in memory_docs:
                    details = score_chunk_details(query, effective_query_tokens, text, preferred_terms=preferred_terms, preferred_paths=preferred_paths, path=path)
                    if details["score"] > 0:
                        hits.append(
                            {
        ...
        hits.sort(
            key=lambda item: (
                order.get(item["source"], 999),
                0 if item.get("strength") == "strong" else 1,
                -float(item.get("score", 0.0)),
                str(item.get("citation", {}).get("path", "")),
                int(item.get("citation", {}).get("lineStart") or 0),
            )
        )
        ...
        for hit in hits:
            citation = hit.get("citation", {})
            path = str(citation.get("path", "unknown"))
            line_start = int(citation.get("lineStart") or 0)
            normalized_snippet = normalize_for_compare(hit.get("snippet", ""))
            duplicate = False
            for existing in deduped:
                existing_citation = existing.get("citation", {})
                if str(existing_citation.get("path", "unknown")) != path:
                    continue
                existing_line_start = int(existing_citation.get("lineStart") or 0)
                if abs(existing_line_start - line_start) <= 8:
                    duplicate = True
                    break
                if normalize_for_compare(existing.get("snippet", "")) == normalized_snippet:
                    duplicate = True
                    break
            if duplicate:
                continue
            if per_path_counts.get(path, 0) >= 2:
                continue
            ...
            if len(deduped) >= 8:
                break
        trimmed = deduped
        strong_hits = [hit for hit in trimmed if hit.get("strength") == "strong"]
        weak_hits = [hit for hit in trimmed if hit.get("strength") != "strong"]
        if trimmed:
            status = "ok"
        elif indexing:
            status = "indexing"
        ...
        response = {
            ...
            "meta": {
                ...
                "effectiveQueryTokens": effective_query_tokens,
                "matchedTerms": ordered_unique(
                    [term for hit in trimmed for term in hit.get("matchedTerms", [])]
                )[:12],
                "strongHitCount": len(strong_hits),
                "weakHitCount": len(weak_hits),
                "topDocumentPaths": ordered_unique(
                    [str(hit.get("citation", {}).get("path", "")) for hit in trimmed if hit.get("citation", {}).get("path")]
                )[:4],
                "followupContextUsed": bool(followup_context),
                "followupEligible": bool(session_id and is_followup_query(query, query_tokens)),
                **({"indexing": {"active": True}} if indexing else {}),
            },
        }
        if status != "ok":
            response["message"] = build_transparency_message(response)
        if session_id:
            self._store_lookup_context(session_id, response)
        return response
